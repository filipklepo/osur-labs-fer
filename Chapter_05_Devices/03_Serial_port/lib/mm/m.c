...
static size_t _mpool_index(size_t size) 
{
    size_t cur_index = 0, next_index_value = INCREMENT;
    while ( size > next_index_value )
    {
        cur_index++;
        next_index_value *= 2;
    }
...
void *m_alloc ( m_mpool_t *mpool, size_t size )
{
    ...
    size_t mpool_index = _mpool_index(size); 
    iter = NULL;
...
{
    size_t mpool_index = _mpool_index(chunk->size);
